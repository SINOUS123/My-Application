...
namespace ProjectVideo.Areas.Identity.Pages.Account
{
    [AllowAnonymous]
    public class RegisterModel : PageModel
    {
        ...
        {
            if(!await _roleManager.RoleExistsAsync(WC.AdminiRoles))
            {
                await _roleManager.CreateAsync(new IdentityRole(WC.AdminiRoles));
        ...
        public async Task<IActionResult> OnPostAsync(string returnUrl = null)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                if (result.Succeeded)
                {
                    if (!await _roleManager.RoleExistsAsync(WC.AdminiRoles))
                    {
                        await _userManager.AddToRoleAsync(user, WC.AdminiRoles);
                    }
                    else
                    {
